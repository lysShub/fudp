...
说明：
	...
	ACT为
	1表示下载文件，此时格式化数据中最后一个key为path(请求资源路径)必须存在(即使是根目录)，path的值不遵守格式化约束;
	2表示上传文件。
...
由Client--->Sever
包头: [0 0 0 2]
paylaod: [非对称加密后的对称加密密钥]
说明：
	Client收到预期的状态码后会发送此包，完成握手。
...
由于UDP本身的不可靠性, 因此可能会出现导致握手失败的情况发生。	采用重试机制提高握手成功率:

1. 握手中, 所有的数据包只发送一次, 如果握手失败, 则重新开始握手
2. 同时增加超时机制, 超时则握手失败；重试次数增加可以提高超时时间

同时，为了即使响应握手结果, Client在发送握手序号包2后应立即开始数据传输、既同样适用超时策略
...
| 0    | 请求完全允许                               |      |
| 1…9  | 也是其他合法请求                           |      |
| 10   | 请求格式错误, 不符合握手包0的格式限制      |      |
| 11   | 请求参数解析错误, 即格式化数据无法正确解析 |      |
| 20   | 没有权限                                   |      |
|      |                                            |      |
|      |                                            |      |
