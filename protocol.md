...
​		协议是CS架构的，S端须先开启端口监听请求、默认端口为19986；客户端发起第一个数据包。S必须先启动等待握手：
...
paylaod: [{证书}]
说明：
	PP模式中不发送证书、即使发送接收端也不会使用; CS模式发送证书，支持自签或CA
```
...
由Client--->Sever
包头: [0 0 0 2]
paylaod: [加密后密钥长度(2B) 非对称加密的密钥 {对称加密的URL的参数部分}]
说明：
	规定非对称加密为ECC_256，摘要为sha256，对称加密方式为AES_GCM_256。
	未加密的的URL采用统一资源定位符的标准格式：[协议类型]://[服务器地址]:[端口号]/[资源层级UNIX文件路径][文件名]?[查询]#[片段ID]。
		协议类型是FUDP或fudp；协议类型、地址和端口可以省略，没有省略会被检查，默认端口为19986；
		路径、文件名、查询、片段ID都对大小写敏感；
		有部分查询字段被保留：
			system：表示C端系统
            time-precision：C端系统的时间精度, 及最小的有效延时的时间长度
        保留字段可选，如果提供则会被检查
        
        
     	ACT为:
	1表示下载文件，此时格式化数据中最后一个key为path(请求资源4路径)必须存在(即使是根目录)，path的值不遵守格式化约束;
	2表示上传文件。
```
...
<font color="#990000">握手中所有数据包大小不能超过协议MTU</font>

<font color="#990000">握手完成之后的的所有数据包将被加密</font>
